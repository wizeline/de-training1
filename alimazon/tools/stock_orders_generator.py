...
def generate_random_stock_orders(settings):
    n_orders = settings['stock_orders_count']
    ...
    for _ in range(n_orders):
        for timestamp in _create_purchase_dates(start_date, end_date, supplier_profile):
            yield _random_stock_order(timestamp, samplers)
...
def _random_stock_order(timestamp, samplers):
...
@click.option(
    '-c', '--conf-file',
    help="""
    JSON format Stock purchase orders configuration file (e.g. stock_orders.conf). If it is not specified, default values will be used.
    """,
    type=str
)
def _smoke_test(conf_file='stock_orders_default.conf'):
    with open(conf_file) as json_conf_file:
        settings = json.load(json_conf_file)

    orders = generate_random_stock_orders(settings)

    def generate_suffix(index):
        return '_{0}_{1:05}'.format(today_string(), index)

    jsonl.write_to_files('./resources/stock-orders/part.jsonl.gz',
                         orders,
