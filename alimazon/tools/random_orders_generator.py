#!/usr/bin/env python
"""Generate random orders records"""
...
_DEFAULT_SAMPLE_SIZE = 15000 #5*10**4
...
_DEFAULT_OUTPUT_FOLDER = './resources/orders'
...
#    codes_sampler = _sampler(codes, _weights(codes, _none=0.3))
    ...
    for idx in range((args['sample_size'] + 1) // args['max_rows']):
        rows = min(args['sample_size'] - args['max_rows'] * idx,
                   args['max_rows'])
        filename = '{}/{}_orders_{:0>5}.jsonl.gz'.format(args['output_folder'],
                                                         _today_string(),
                                                         idx)
...
#                'country': next(codes_sampler),
                'registration_date': (args['start'] + timedelta(args['period'] * random())).isoformat()
            })
            for _ in range(rows)))
            output_file.write('\n')
